...
+ (void)load {
    if (@available(iOS 11.0, *)) {
        Method m1 = class_getInstanceMethod(self.class, @selector(safeAreaInsets));
        Method m2 = class_getInstanceMethod(self.class, @selector(safeAreaInsets2));
        method_exchangeImplementations(m1, m2);
    } else {
        class_addMethod(self.class, @selector(safeAreaInsets), imp_implementationWithBlock(^(){return UIEdgeInsetsZero;}), "UIEdgeInsets=dddd@:");
    }
}
...
- (UIView *)firstResponder {
    ...
    for (UIView *view in self.subviews) {
        ...
        ddd = [view firstResponder];
    }
    return ddd;
}

- (UIEdgeInsets)safeAreaInsets2 {
    return [self safeAreaInsets2];
}
